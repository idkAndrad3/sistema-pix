...
/**
 * Interface gráfica para o servidor PIX integrado com banco de dados. Abas:
 * Log, Usuários, Transações, Clientes
 */
public class ServerGUI extends JFrame {
	...
	private void setupLayout() {
		...
		DefaultTableModel userModel = new DefaultTableModel(
				new Object[] { "CPF", "Nome", "Saldo", "Criado Em", "Atualizado Em" }, 0) {
			@Override
		...
		DefaultTableModel txModel = new DefaultTableModel(
				new Object[] { "ID", "Valor", "CPF Enviador", "CPF Recebedor", "Data" }, 0) {
			@Override
			public boolean isCellEditable(int row, int column) {
				return false;
			}
		...
		DefaultTableModel clientsModel = new DefaultTableModel(
				new Object[] { "ID Cliente", "IP", "Porta", "Hostname", "Status", "Conectado Em" }, 0) {
			@Override
			public boolean isCellEditable(int row, int column) {
				return false;
			}
	...
	private void startServer() {
		try {
			...
			JOptionPane.showMessageDialog(this, "Erro ao iniciar servidor: " + ex.getMessage(), "Erro",
	...
	private void stopServer() {
		try {
			...
			JOptionPane.showMessageDialog(this, "Erro ao parar servidor: " + ex.getMessage(), "Erro",
					JOptionPane.ERROR_MESSAGE);
	...
	private void refreshClientes() {
		try {
			...
			for (Sessao s : lista) {
				...
				model.addRow(
						new Object[] { s.getId(), s.getIp(), s.getPorta(), s.getHostname(), s.getStatus(), conectado });
			}
