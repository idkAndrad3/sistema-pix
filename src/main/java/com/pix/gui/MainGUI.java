...
public class MainGUI extends JFrame {
	...
	private void initializeComponents() {
		...
		deletarButton.setBackground(new Color(220, 50, 50));
	...
	private JPanel createPixPanel() {
		...
		// Área de instruções
		gbc.gridy = 3;
		JTextArea instructions = new JTextArea("Instruções:\n" + "1. Digite o valor a ser enviado\n"
	...
	private JPanel createConfigPanel() {
		...
		// Aviso
		gbc.gridy = 3;
		JTextArea warning = new JTextArea("Atenção:\n" + "• Deixe em branco os campos que não deseja alterar\n"
	...
	private void carregarDadosUsuario() {
		...
		if (result.isSuccess()) {
			...
			JOptionPane.showMessageDialog(this, "Erro ao carregar dados: " + result.getMessage(), "Erro",
	...
	private void enviarPix() {
		try {
			...
			if (cpfDestino.length() != 14) {
				JOptionPane.showMessageDialog(this, "CPF inválido: informe 11 dígitos.", "Erro",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if (result.isSuccess()) {
				JOptionPane.showMessageDialog(this, "PIX enviado com sucesso para " + cpfDestino, "Sucesso",
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(this, "Erro ao enviar PIX: " + e.getMessage(), "Erro",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void realizarDeposito() {
		try {
			...
			if (valor <= 0) {
				JOptionPane.showMessageDialog(this, "O valor do depósito deve ser positivo!", "Erro",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if (result.isSuccess()) {
				JOptionPane.showMessageDialog(this, "Depósito realizado com sucesso!", "Sucesso",
						JOptionPane.INFORMATION_MESSAGE);
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(this, "Erro ao realizar depósito: " + e.getMessage(), "Erro",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void atualizarDados() {
		...
		if (novoNome.isEmpty() && novaSenha.isEmpty()) {
			JOptionPane.showMessageDialog(this, "Informe pelo menos um campo para atualizar!", "Erro",
					JOptionPane.ERROR_MESSAGE);
			return;
		}

		PixClient.OperationResult result = client.atualizarUsuario(token, novoNome.isEmpty() ? null : novoNome,
	...
	private void carregarExtrato() {
		...
		if (client == null || token == null || token.trim().isEmpty()) {
			JOptionPane.showMessageDialog(this, "Cliente não inicializado ou token inválido", "Erro",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
				...
				try {
					if (result != null && result.isSuccess() && result.getTransacoes() != null) {
						...
						for (JsonNode transacao : transacoes) {
							...
							} else if (!transacao.path("data_hora").isMissingNode()
									&& !transacao.path("data_hora").asText().isEmpty()) {
							...
							String origem_destino = String.format("%s (%s) -> %s (%s)",
									nomeEnviador.isEmpty() ? cpfEnviador : nomeEnviador, cpfEnviador,
									nomeRecebedor.isEmpty() ? cpfRecebedor : nomeRecebedor, cpfRecebedor);
						...
						JOptionPane.showMessageDialog(MainGUI.this, "Não foi possível carregar extrato: " + msg, "Erro",
								JOptionPane.ERROR_MESSAGE);
					}
				} catch (Exception e) {
					e.printStackTrace();
					JOptionPane.showMessageDialog(MainGUI.this, "Erro ao processar extrato: " + e.getMessage(), "Erro",
							JOptionPane.ERROR_MESSAGE);
				} finally {
	...
	private void realizarLogout() {
		int option = JOptionPane.showConfirmDialog(this, "Deseja realmente sair?", "Confirmar Logout",
	...
	private static class CurrencyDocumentFilter extends DocumentFilter {
		...
		public void replace(FilterBypass fb, int offset, int length, String text, AttributeSet attrs)
			...
			// comportamento simples: se houve substituição, retiramos 'length' dígitos do
			// final e então anexamos os novos
	...
	private void deletarConta() {
		int confirm = JOptionPane.showConfirmDialog(this,
				"Tem certeza que deseja deletar sua conta? Esta ação é irreversível.", "Confirmar Exclusão",
				JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);

		if (confirm == JOptionPane.YES_OPTION) {
			PixClient.OperationResult result = client.deletarUsuario(token);

			if (result.isSuccess()) {
				JOptionPane.showMessageDialog(this, "Sua conta foi deletada com sucesso.", "Sucesso",
						JOptionPane.INFORMATION_MESSAGE);

				// Força o logout e fecha a aplicação
				client.disconnect();
				...
				JOptionPane.showMessageDialog(this, "Erro ao deletar conta: " + result.getMessage(), "Erro",
						JOptionPane.ERROR_MESSAGE);
			}
		}
	}
